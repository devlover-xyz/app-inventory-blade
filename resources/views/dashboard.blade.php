<x-layouts.app :title="__('Dashboard')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        ...
        <p>
            {{ __('Welcome to your dashboard.') }} <br>
            {{ __('Start managing your inventory, transactions, and users.') }} <br>
            {{__(' Anda login sebagai:')}} <strong>{{ auth()->user()->getRoleNames()->implode(', ') }}</strong>
        </p>
    </div>
</x-layouts.app>
